...
public class PlayerSaver {
    ...
    public static void saveInventory() {
        ...
        for(int i = 0; i < PLAYER_INVENTROY_SIZE; i++) {
            ItemStack temp = Minecraft.getInstance().player.inventory.getStackInSlot(i);
             SAVED_PLAYER_ITEMS.add(temp);
            Minecraft.getInstance().player.sendChatMessage("Items " + SAVED_PLAYER_ITEMS.get(i) + " saved");
        }
    ...
    public static void loadInventory() {
        if(SAVED_PLAYER_ITEMS.size() != 0) {
            for(int i = 0; i < PLAYER_INVENTROY_SIZE; i++) {
                Minecraft.getInstance().player.inventory.setInventorySlotContents(i, SAVED_PLAYER_ITEMS.get(i));
